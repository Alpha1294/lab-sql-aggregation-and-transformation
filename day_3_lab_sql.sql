...
SELECT 
	FLOOR(AVG(length) / 60) AS avg_hours, -- floor always round down
    ROUND(AVG(length) % 60) AS avg_minutes -- round returns the nearest integer to teh given number.
FROM film;
...
FROM film
ORDER BY title ASC;
-- 4 Bonus: The marketing team for the movie rental company now needs to create a personalized email campaign for customers. To achieve this, you need to retrieve the concatenated first
--  and last names of customers, along with the first 3 characters of their email address, so that you can address them by 
...
SELECT 
    ...
    SUBSTR(email,1,3) as email_start
FROM
	customer
...
FROM film
GROUP BY rating;
-- 1.3 The number of films for each rating, sorting the results in descending order of the number of films. This will help you to better understand the
